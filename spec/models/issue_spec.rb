...
RSpec.describe Issue, type: :model do
  describe "Validation" do
    ...
    it "Checks if issues creation throws validation error when title, customer and description are blank" do
      @issue = Issue.create
    ...
    it "Checks if issues creation throws validation error when description is blank" do
      @issue = Issue.create(customer_id: @customer.id, title: "Not able to start")
      expect(@issue.errors.messages[:description]).to eq ["can't be blank"]
    end

    it "Checks if issues creation throws validation error when title is blank" do
      @issue = Issue.create(customer_id: @customer.id, description: "It's very complicated")
      expect(@issue.errors.messages[:title]).to eq ["can't be blank"]
    end

    it "Checks if issues are getting created with correct values and correct status got assigned" do
